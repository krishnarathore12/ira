...
def get_relevant_context(agent_a, agent_b, input_dialogue, embeddings, sess_id, context_length=2, reflection=False):
    ...
    # Guard against occasional embedding/context length drift.
    top_k_sims_a = [idx for idx in top_k_sims_a if idx < len(contexts_a)]
    top_k_sims_b = [idx for idx in top_k_sims_b if idx < len(context_b)]
    if len(top_k_sims_a) == 0:
        top_k_sims_a = list(range(max(0, len(contexts_a) - context_length), len(contexts_a)))
    if len(top_k_sims_b) == 0:
        top_k_sims_b = list(range(max(0, len(context_b) - context_length), len(context_b)))
