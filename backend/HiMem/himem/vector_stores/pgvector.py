...
class PGVector(VectorStoreBase):
    ...
    def insert(self, docs: list, embeddings: list[list[float]], payloads=None, ids=None) -> None:
        logger.info(f"Inserting {len(embeddings)} vectors into collection {self.collection_name}")
        json_payloads = [json.dumps(payload) for payload in payloads]

        data = [(id, vector, payload) for id, vector, payload in zip(ids, embeddings, json_payloads)]
        if PSYCOPG_VERSION == 3:
            with self._get_cursor(commit=True) as cur:
                cur.executemany(
    ...
    def search(
        ...
        Args:
            query (str): Query.
            query_embeddings (List[float]): Query vector.
        ...
        with self._get_cursor() as cur:
            cur.execute(
                ...
                LIMIT %s
                """,
                (query_embeddings, *filter_params, limit),
            )
    ...
    def update(
        ...
        Args:
            ...
            doc (str): Text.
            embeddings (List[float], optional): Updated vector.
